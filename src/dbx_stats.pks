CREATE OR REPLACE PACKAGE dbx_stats AUTHID CURRENT_USER AS
    ...
    PROCEDURE create_watcher_job(g_session_id VARCHAR2, v_max_parallel_jobs number, v_instance_count NUMBER);
    ...
    PROCEDURE gather_stale_objects(schema_name VARCHAR2, objlist OUT dbms_stats.objecttab, p_degree IN NUMBER);
    PROCEDURE gather_empty_objects(schema_name VARCHAR2, objlist OUT dbms_stats.objecttab, p_degree IN NUMBER);
    ...
    FUNCTION get_stale_stats_schema(v_schema_to_check VARCHAR2, p_degree IN NUMBER default 4) RETURN dbx_stale_stats_table PIPELINED;
    ...
    PROCEDURE watch_jobs(v_g_session_id VARCHAR2, v_max_parallel_jobs number, v_instance_count NUMBER); -- Updated watch_jobs procedure to accept g_session_id
